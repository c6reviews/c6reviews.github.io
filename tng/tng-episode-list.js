		...
		if (td) {
			...
			if (normalizeText(txtValue).indexOf(filter) > -1) {
				...
				if (filter != "") {
					...
					if (linktext === "Ménage à Troi" && normalizeText(linktext).indexOf(filter) > -1) {
						const accentedFilter = filter.replace("e","é").replace(" a"," à").replace("a ","à ");
