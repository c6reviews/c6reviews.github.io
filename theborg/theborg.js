...
/* ----------------------------------------------------- GLOBAL VARIABLES ----------------------------------------------------- */
								// NUMBER BY DISPLAYED COLUMNS ONLY, STARTING AT 0
const G_seriescol = 3; 			// <-- SET THIS TO THE NUMBER CORRESPONDING TO THE SERIES COLUMN
const G_episodecol = 4;			// <-- SET THIS TO THE NUMBER CORRESPONDING TO THE EPSIDOE NUMBER COLUMN
const G_titlecol = 5;  			// <-- SET THIS TO THE NUMBER CORRESPONDING TO THE TITLE COLUMN
const G_recommendationcol = 7;	// <-- SET THIS TO THE NUMBER CORRESPONDING TO THE RECOMMENDATION COLUMN
const G_rolecol = 2;
...
function displaySpoiler(caller,blurRadius = '6px') {
...
async function createTable(showSpoiler) {
	...
	const csvResponse = await fetch("theborg.csv");
		...
		for (var i = 0; i < numColumns; i++) {
			...
			switch (i) {
				case 0:
					...
					continue;
				case 1:
					cellClass = "col_releaseDate";
					break;
				case 2:
					cellClass = "col_inUniverseDate";
					cell = cell.replace(/(\d)(\...)/,'$1<span style="font-size:.625rem;color:#606060;font-style:italic">$2</span>');
					break;
				case 3:
					cellClass = "col_role";
					switch (cell) {
						case 'P':
							cell = '<strong style="cursor:default" title="Primary">P</strong>';
							break;
						case 'S':
							cell = '<span style="cursor:default" title="Secondary">S</span>';
							break;
						case 'M':
							cell = '<span style="cursor:default" title="Minor">M</span>';
							break;
						case 'VM':
							cell = '<span style="cursor:default" title="Very Minor">VM</span>';
							break;						
					}
					break;
				case 4:
					...
					cell = cell.replace('ENT','<img src="../images/ent-abbr.webp" style="height:15px;" alt="ENT" title="Enterprise">');
					cell = cell.replace('LOW','<img src="../images/low-abbr.webp" style="height:15px;" alt="LOW" title="Lower Decks">');
					cell = cell.replace('PRO','<img src="../images/pro-abbr.webp" style="height:22px;" alt="PRO" title="Prodigy">');
	...
	epNumCells.forEach((cell) => {
		...
		if (cellContents.includes("#")) {
			var link = "/films/index.html?goToFilm=" + cellContents.slice(1);
		} else {		
			var season = cellContents.substring(0,1);
			var episode = cellContents.substring(2,4);
			var series = cell.parentElement.querySelectorAll("td")[G_seriescol].querySelector("img").alt.toLowerCase();
			var link = "/" + series + "/" + series + "-s" + season + ".html#e" + episode;
		}
		
		var titleCell = cell.parentElement.querySelectorAll("td")[G_titlecol];
	...
	while (switching) {
		...
		for (i = 0; i < (rows.length - 1); i++) {
			...
			if (!!isNumber){
				...
				// Special Sort for Role column
				if (n == G_rolecol) {
					switch (x.innerText.toLowerCase()) {
						case "p":
							x = 1;
							break;
						case "s":
					...
					switch (y.innerText.toLowerCase()) {
						case "p":
							y = 1;
							break;
						case "s":
...
function setRnFilters() {
	...
	filterrows.forEach((filterrow) => {
		...
		var checkCell = filterrow.getElementsByTagName("td")[G_recommendationcol];
...
function setFilters(type) {
	...
	else if (type=="rn") // A recommendation filter was checked or unchecked
	{
	...
	else if (type=="hideMinorRows") {
		
		var hideMinorRows = document.getElementById("hideMinorRows").checked;
		...
		if (hideMinorRows) {
			...
			Array.from(G_filterset).forEach((filterrow) => {
				...
				if ((checkCell.innerText).includes("M")){
					filterrow.style.display="none";
				}
			});
...
function resetFilters() {
	...
	document.getElementById("hideMinorRows").checked = false;
...
function addSeasonSeparator() {			// THIS FUNCTION HAS BEEN REPURPOSED ON THIS PAGE TO "GROUP" THE TWO ENTRIES FOR Star Trek: First Contact
	table = document.getElementById("episodeTable");
	rows = table.rows;
	...
	for (i = 0; i < (rows.length - 1); i++) {
		...
		x = rows[i].getElementsByTagName("TD")[G_titlecol];
		if (x.innerText != "Star Trek: First Contact") {continue;}
		
		for (j = i + 1; j < (rows.length); j++) {
			
			y = rows[j].getElementsByTagName("TD")[G_titlecol];
