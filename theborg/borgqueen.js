...
const csvsource = "borgqueen.csv";
									// NUMBER BY DISPLAYED COLUMNS ONLY, STARTING AT 0
const G_seriescol = 2; 				// <-- SET THIS TO THE NUMBER CORRESPONDING TO THE SERIES COLUMN
const G_episodecol = 3;				// <-- SET THIS TO THE NUMBER CORRESPONDING TO THE EPSIDOE NUMBER COLUMN
const G_titlecol = 4;  				// <-- SET THIS TO THE NUMBER CORRESPONDING TO THE TITLE COLUMN
const G_recommendationcol = 7;		// <-- SET THIS TO THE NUMBER CORRESPONDING TO THE RECOMMENDATION COLUMN

const G_totalRowsNoSpoilers = 17;	// Not including header row(s)
const G_totalRowsWithSpoilers = 21; // Not including header row(s)
...
function displaySpoiler(caller,blurRadius = 6) {
	
	var content = caller.parentNode.querySelector('.spoilerContent');
	var style = getComputedStyle(content);
	var blurPx = blurRadius + 'px';
	
	if (style.filter == "blur("+blurPx+")") {
		content.style.filter = "none";
		content.style.userSelect = "auto";
		content.style.pointerEvents = "auto";
	} else {
		content.style.filter = "blur("+blurPx+")";
...
async function createTable(showSpoiler) {
	...
	if (showSpoiler == true) {
		var rows = G_totalRowsWithSpoilers;
	} else {
		var rows = G_totalRowsNoSpoilers;
	}
	var content = ``;
...
function addSeasonSeparator() {			// THIS FUNCTION HAS BEEN REPURPOSED ON THIS PAGE TO "GROUP" THE TWO ENTRIES FOR Star Trek: First Contact
	...
	for (i = 0; i < (rows.length - 1); i++) {
		...
		x = rows[i].getElementsByTagName("TD")[G_episodecol];
		
		for (j = i + 1; j < (rows.length - 1); j++) {
			
			y = rows[j].getElementsByTagName("TD")[G_episodecol];
