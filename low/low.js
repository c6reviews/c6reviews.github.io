...
var G_galleryIndex = 1;

function galleryNext() {
	G_galleryIndex++;
	var galleryImg = document.getElementById("galleryImg");
	galleryImg.src =  "images/orion-" + G_galleryIndex + ".webp";
	if (G_galleryIndex > 1)
	{
		document.getElementById("galleryLeft").style.display = "block";
	} else {
		document.getElementById("galleryLeft").style.display = "none";
	}
	if (G_galleryIndex < 5)
	{
		document.getElementById("galleryRight").style.display = "block";
	} else {
		document.getElementById("galleryRight").style.display = "none";
	}
	
	switch (G_galleryIndex) {
		case 1:
			galleryImg.alt = "A wedding announcement written in an alien language with the translation added to the image later: 'TOGETHER WITH OUR FAMILY / SOME TEXT HERE / DERIKA TENDI & FIANCE ORION / PLACE HOLDER TEXT / WEDDING / CAS WED'";
			break;
		case 2:
			galleryImg.alt = "Mariner and Tendi being placed on a sedan chair with alien text inscribed on it, and the translation added to the image later: 'THIS IS A VERY FANCY CHAIR'";
			break;
		case 3:
			galleryImg.alt = "A city block with several signs written in alien text, with translations added to the image later: 'SLIT THROAT / DRINK DANCE STEAL / SHOP / COFFEE'";
			break;
		case 4:
			galleryImg.alt = "The club entrance with a sign in alien text, with the translation added to the image later: 'SLIT THROAT'";
			break;
		case 5:
			galleryImg.alt = "The bartender pouring a drink from a bottle with alien text, with translations added to the image later: 'BRANDY / CAPTAINS CHOICE'";
			break;
	}
}
...
window.onload = function() {
	...
	// ************************* TOOL TIPS *************************
			...
			// Movie Poster Tool Tip
			let ele = document.getElementById('movieposter');
			if (ele){
				ele.onclick = function(e){
				var tooltip = document.getElementById('movieposterToolTip');
				var x = e.clientX,
				y = e.clientY;
